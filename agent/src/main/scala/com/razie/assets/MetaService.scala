...
class MetaService extends AgentService {
   ...
   def objects() =  
      razie.Draw attrTable AssetMgr.metas.toList.sortWith(_<_).map(razie.AA ("name", _))
   ...
   def classes(out:DrawStream, format:String) =  {
      format match {
         case "json" => {
            ...
            for (s <- AssetMgr.metas.toList.sortWith(_<_)) 
      ...
      out write Draw.seq (
         ...
         Draw.attrTable (
            for (s <- AssetMgr.metas.toList.sortWith(_<_)) yield {
               val m = Metas.meta(s)
   ...
   def services() = 
      razie.Draw attrTable (
         for (s <- (razie.M apply com.razie.pub.agent.Agent.instance().copyOfServices()).sort(_.getClass().getSimpleName()<_.getClass().getSimpleName())) yield
